"""Generic CLI worker agent wrapper.

Launches a configurable CLI agent (default: claude) as a subprocess to execute a task.
The wrapper handles branching, heartbeat, instruction building, CLI invocation, commits,
events, and memory. Which CLI to call is determined by the WORKER_BACKEND env var or
permissions/backends.yaml. Use 'orchctl config set worker-backend <name>' to change it.

  1. Creates the agent branch via the gateway.
  2. Invokes the configured CLI backend with the task instruction.
  3. On success: commits changed files via the gateway and marks the task completed.
  4. On failure: marks the task failed.

Usage:
    uv run python -m agents.worker.main \\
...
app = typer.Typer(name="worker-agent", add_completion=False)
...
def _build_instruction(pkg: dict, repo_path: str) -> str:
    task = pkg["task"]
    task_owner: str = task.get("owner", "worker")
    ...
    # Inline the pre-loaded input artifact content so the agent doesn't need extra reads.
    artifact_section = ""
...
{auth_hdr}      -d '{{
        ...
        "payload": {{
          ...
          "owner_hint": "<backend-agent|frontend-agent|qa-agent|devops-agent|human>",
...
def _mark_suspended(
    ...
    if not commits:
        ...
        _mark_failed(http, orch_url, task_id, reason, actor=task_owner)
        return
...
def main(
    ...
    """Run the configured CLI backend agent for a given context package."""
    ...
    log.info("Worker agent starting: task=%s branch=%s", task_id, branch)
    ...
    with httpx.Client(timeout=30.0) as http:
        ...
        # ── 2. Run the configured CLI backend ─────────────────────────────
        instruction = _build_instruction(pkg, repo_path)
        _backend_name = get_backend("worker")
        _backend_cfg = load_backends().get(_backend_name, {})
        log.info("Launching backend '%s' (timeout 1800s)...", _backend_name)

        # For the CLI invocation we need direct subprocess control (cwd, env, timeout)
        # so we replicate cli_runner logic here rather than calling run_cli_prompt,
        # which doesn't support cwd. We still use get_backend/load_backends for config.
        _cli_command: list[str] = _backend_cfg.get(
            "command", ["claude", "--dangerously-skip-permissions", "-p", "-"]
        )
        _input_mode: str = _backend_cfg.get("input", "stdin")
        _cli_env = {k: v for k, v in os.environ.items() if k != "ANTHROPIC_API_KEY"}

        _tmp_prompt_path: str | None = None
        try:
            if _input_mode == "stdin":
            ...
            elif _input_mode == "arg":
                result = subprocess.run(
                    _cli_command + [instruction],
                    cwd=repo_path,
                    env=_cli_env,
                    capture_output=True,
                    text=True,
                ...
                with tempfile.NamedTemporaryFile(
                    ...
                    _tf.write(instruction)
                    _tmp_prompt_path = _tf.name
                result = subprocess.run(
                    _cli_command + [_tmp_prompt_path],
                    cwd=repo_path,
                    env=_cli_env,
                    capture_output=True,
                    text=True,
            ...
            hb_stop.set()
            _exe = _cli_command[0] if _cli_command else _backend_name
            log.error("CLI backend '%s' not found (command: %s).", _backend_name, _exe)
            _mark_failed(
                http, orch_url, task_id, f"backend:{_backend_name}:not_found", actor=task_owner
            )
            raise typer.Exit(1)
        except subprocess.TimeoutExpired:
            hb_stop.set()
            log.error("Backend '%s' timed out after 1800s", _backend_name)
            _mark_suspended(
                http,
                gw_url,
                orch_url,
                ...
                repo_path,
                _auth_hdrs,
                f"backend:{_backend_name}:timeout:1800s",
            )
            raise typer.Exit(1)
        finally:
            if _tmp_prompt_path:
                try:
                    os.unlink(_tmp_prompt_path)
                except OSError:
                    pass
        ...
        if result.returncode != 0:
            ...
            if any(p in _combined_lower for p in _ENV_LIMIT_PATTERNS):
                log.warning(
                    "env_limitation detected in backend output; escalating task %s to awaiting_human",
                    task_id,
                )
                try:
                    http.post(
                        ...
                        json={
                            ...
                            "question": (
                                f"Backend '{_backend_name}' exited {result.returncode} with an "
                                "environment limitation (session limit, rate limit, or auth "
                                "failure). Resolve the issue and respond to re-run."
                            ),
            ...
            _mark_suspended(
                ...
                repo_path,
                _auth_hdrs,
                f"backend:{_backend_name}:exit_{result.returncode}:{_hint}",
            )
            raise typer.Exit(1)

        log.info("Backend '%s' finished successfully", _backend_name)
        ...
        if not changed_paths:
            ...
            if prior_count > 0:
                ...
                hb_stop.set()
                log.warning("Backend exited 0 but no files changed; marking task failed")
                _mark_failed(http, orch_url, task_id, "no_files_changed", actor=task_owner)
                raise typer.Exit(1)
        ...
        try:
            _call(
                ...
                json={
                    "agent_id": task_owner,
                    "task_id": task_id,
                    "event_type": "WORKER_FILES_WRITTEN",
                    "payload": {
                        "paths": changed_paths,
                        "sha": sha,
                        "run_id": run_id,
        ...
        try:
            _call(
                ...
                "POST",
                f"{orch_url}/tasks/{task_id}/transition",
                json={"new_status": "completed", "actor": task_owner},
            )
    ...
    log.info("Worker agent done: task=%s", task_id)
