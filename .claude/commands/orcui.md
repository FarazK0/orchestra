...
- `/orcui show agent identities`
- `/orcui teach backend-agent "always use select() not query()"`
...
| why did task N fail | `uv run python -m cli.main why TASK-00N` |
| resume task N | `uv run python -m cli.main resume TASK-00N` |
| show questions / what are agents asking | `uv run python -m cli.main questions` |
| respond to task N / answer TASK-00N | `uv run python -m cli.main respond TASK-00N "<answer>"` |
| recover task N | `uv run python -m cli.main recover TASK-00N` |
| show validators / list validators | `uv run python -m cli.main validator list` |
...
| show skill memory / list skills | `uv run python -m cli.main memory list --type skill` |
| show identity / show identities | `uv run python -m cli.main identities` |
| show identity for \<agent\> | `uv run python -m cli.main identities --agent <agent>` |
| teach \<agent\> "\<fact\>" / inject skill | `uv run python -m cli.main teach <agent> "<fact>" --topic <topic>` |
| forget \<agent\> \<topic\> / remove skill | `uv run python -m cli.main forget <agent> <topic>` |
| ask \<agent\> "\<question\>" | `uv run python -m cli.main ask <agent> "<question>"` |
| session with \<agent\> / talk to \<agent\> | `uv run python -m cli.main session <agent>` |
| show config / llm backend | `uv run python -m cli.main config show` |
| set llm backend / use claude or python | `uv run python -m cli.main config set llm-backend <claude\|python>` |
...
What would you like to do?
  ...
  Task flow
    /orcui show <status> tasks         — filter by status (running / failed / escalated / awaiting_human / suspended)
    /orcui show task N                 — full detail for one task including validation history
    /orcui approve TASK-00N            — advance through approval gate
    /orcui validate TASK-00N           — run all assigned validators on agent branch
    /orcui merge TASK-00N              — merge validated branch into main
    /orcui review                      — interactive approval loop for all ready tasks
    /orcui show questions              — list tasks waiting for human answers
    /orcui respond TASK-00N "answer"   — send an answer to a waiting agent
    /orcui resume TASK-00N             — re-queue a suspended task
  ...
  Agent identities
    /orcui show identities             — list all agent profiles + domain expertise
  ...
  Validators and config
    ...
    /orcui show config                 — show current CLI config (llm-backend etc.)
    /orcui set llm backend claude      — switch ask/session to use claude CLI
    /orcui set llm backend python      — switch ask/session to use Python LLM client
  ...
  If something went wrong
    /orcui why TASK-00N                — show failure diagnostics
    /orcui recover TASK-00N            — mark escalated task as completed for manual merge
...
- **`awaiting_human` tasks:** flag clearly — the agent is blocked waiting for a human answer. Suggest `/orcui show questions`.
- **`suspended` tasks:** flag clearly — the agent was interrupted. Suggest `/orcui resume TASK-00N`.
...
| Task is `awaiting_human` | Answer with `/orcui respond TASK-00N "answer"` or `/orcui show questions` |
| Task is `suspended` | Resume with `/orcui resume TASK-00N` |
| Task is `failed` | Check logs; cancel or retry |
| Task is `escalated` | Check logs; try `/orcui why TASK-00N`; cancel and re-request |
...
- **Remove a human-taught skill:**
  ```bash
  uv run python -m cli.main forget <agent> <topic> --yes
  ```
...
- `orchctl ask` and `orchctl session` branch by agent type: `claude-code-agent` uses the `claude` CLI subprocess (needs `claude login`, not `ANTHROPIC_API_KEY`); all other agent types use LLMClient directly (needs `ANTHROPIC_API_KEY`). Warn the user if the required auth is missing.
